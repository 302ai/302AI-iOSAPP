...
@Observable class ImageGeneration: Identifiable, Hashable, Equatable {
    ...
    func send(query: ImagesQuery, configuration: ImageSession.Configuration) async {
        ...
        let api = ApiDataManager.shared.selectedItem //ApiItemStore().currentItem //AppState.shared.loadSelection()
        let openAIconfig = OpenAI.Configuration(token: api?.apiKey ?? "", host: api?.host ?? "")
