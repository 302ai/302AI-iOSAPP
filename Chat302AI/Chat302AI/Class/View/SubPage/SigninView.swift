...
struct SigninView: View {
    ...
    @State private var isShowForgetPwd = false
                ...
                VStack(spacing: 20) {
                    ...
                    VStack(spacing: 20) {
                        ...
                        VStack{
                            // 登录方式选择
                            HStack(spacing: 100) {
                                loginButton(title: "短信登录".localized(), index: 0)
                                loginButton(title: "密码登录".localized(), index: 1)
                            }
                        ...
                        if selectedTab == 0 {
                            ...
                            VStack(alignment: .leading, spacing: 8) {
                                ...
                                HStack{
                                    Text("手机号/邮箱".localized())
                                    Spacer()
                                }
                                TextField("请输入邮箱/手机号".localized(), text: $email)
                                    .textFieldStyle(PlainTextFieldStyle())
                                    .padding(.horizontal, 12)
                                    .padding(.vertical, 10)
                                    .background(Color.gray.opacity(0.1))
...
//                )
                ...
                .sheet(isPresented: $isShowForgetPwd) {
                    ForgetPwdView(apiKey: $apiKey, username: $username, uid: $uid, userInfo: $userInfo)
                }
    ...
    //MARK: - 验证码登录
    private func sendSMSLoginRequest(  ) {
        
        responseMessage = ""
        //(smsCode: msgCode, phoneNumber: "\(country?.dialingCode ?? "+86")\(phoneNumber)")
        
        NetworkManager.shared.msgCodeLoginWithPhone(sms_code: msgCode, phone_number: "\(country?.dialingCode ?? "+86")\(phoneNumber)") { result in
            DispatchQueue.main.async {
                switch result {
                case .success(let response):
                    if response.code == 0 {
    ...
    //密码登录
    private func sendPasswdLoginRequest(  ) {
        NetworkManager.shared.userLoginWithAll(account: email, password: password, captcha: verificationCode , code: responseCode ) { result in
