...
// absolute
func saveImage(image: NSImage, fileName: String = Date().nowFileName(), inFolder folderName: String = "302.AI_Images") -> String? {
...
// relative
func saveImage(image: UIImage, fileName: String = Date().nowFileName(), inFolder folderName: String = "302.AI_Images") -> String? {
...
//func loadImage(from filePath: String) -> UIImage? {
    ...
    func loadImage(from filePath: String) -> UIImage? {
        if filePath.hasPrefix("file://") {
            let filePath2 = URL(string: filePath)?.path ?? ""
            return UIImage(contentsOfFile: filePath2)
        } else {
            // 处理其他类型的 URL（网络 URL 或应用内资源）
